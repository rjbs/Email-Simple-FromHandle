# This Makefile is for the Email::Simple::FromHandle extension to perl.
#
...
#     DIR => []
#     DISTNAME => q[Email-Simple-FromHandle]
#     NAME => q[Email::Simple::FromHandle]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { IO::String=>q[0], Email::Simple=>q[1.9] }
#     VERSION => q[0.011]
...
NAME_SYM = Email_Simple_FromHandle
VERSION = 0.011
...
FULLEXT = Email/Simple/FromHandle
BASEEXT = FromHandle
PARENT_NAME = Email::Simple
DLBASE = $(BASEEXT)
VERSION_FROM = 
OBJECT = 
LDFROM = $(OBJECT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Email/Simple/FromHandle.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Email/Simple
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Email/Simple
...
TO_INST_PM = lib/Email/Simple/FromHandle.pm

PM_TO_BLIB = lib/Email/Simple/FromHandle.pm \
...
DISTNAME = Email-Simple-FromHandle
DISTVNAME = Email-Simple-FromHandle-0.011
...
manifypods : pure_all  \
	lib/Email/Simple/FromHandle.pm \
	lib/Email/Simple/FromHandle.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Email/Simple/FromHandle.pm $(INST_MAN3DIR)/Email::Simple::FromHandle.$(MAN3EXT) 
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,011,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Ricardo SIGNES &lt;rjbs@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Email-Simple" VERSION="1,9,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/Email/Simple/FromHandle.pm blib/lib/Email/Simple/FromHandle.pm 
